...
```bash
uv run SKILL_DIR/scripts/xlsx_unpack.py input.xlsx /tmp/xlsx_work/
...
uv run SKILL_DIR/scripts/xlsx_pack.py /tmp/xlsx_work/ output.xlsx
```

**Add a column** (formulas, numfmt, styles auto-copied from adjacent column):
```bash
uv run SKILL_DIR/scripts/xlsx_unpack.py input.xlsx /tmp/xlsx_work/
uv run SKILL_DIR/scripts/xlsx_add_column.py /tmp/xlsx_work/ --col G \
...
uv run SKILL_DIR/scripts/xlsx_insert_row.py /tmp/xlsx_work/ --at 5 \
...
uv run SKILL_DIR/scripts/xlsx_reader.py input.xlsx                 # structure discovery

# Scripts without dependencies work with both uv run and python3
uv run SKILL_DIR/scripts/formula_check.py file.xlsx --json         # formula validation
uv run SKILL_DIR/scripts/formula_check.py file.xlsx --report       # standardized report
uv run SKILL_DIR/scripts/xlsx_unpack.py in.xlsx /tmp/work/         # unpack for XML editing
uv run SKILL_DIR/scripts/xlsx_pack.py /tmp/work/ out.xlsx          # repack after editing
uv run SKILL_DIR/scripts/xlsx_shift_rows.py /tmp/work/ insert 5 1  # shift rows for insertion
uv run SKILL_DIR/scripts/xlsx_add_column.py /tmp/work/ --col G ... # add column with formulas
uv run SKILL_DIR/scripts/xlsx_insert_row.py /tmp/work/ --at 6 ...  # insert row with data
```
