...
meta:
  ...
  version: 1.0.0
  description: |
    这是一个示例 SOP，展示了基本的工作流程结构。
    修改它以适应你的需求。

# 全局配置
config:
  # 对抗模式: FAIL 必须修复，不允许跳过
...
doctor() {
    ...
    echo "=== 文件检查 ==="
    for file in "SKILL.md" "bootstrap.sh" "engines/workflow-engine.js" "engines/verifier-prompts.js" "manifests/superpowers-sop.yaml"; do
