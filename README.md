...
| `sop-workflow` | Cross-platform multi-agent workflow execution with adversarial verification. DAG-based parallel task waves (Kahn's algorithm), two-stage review (spec compliance → code quality), wave-level AbortController cancellation, atomic state persistence with crash recovery, exponential backoff retries, and evidence-before-claims shell command injection into verifier prompts. Powered by MiniMax-M1. | Community |
